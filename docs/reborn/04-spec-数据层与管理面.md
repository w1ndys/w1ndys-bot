...
3. `internal/webapi`：
   - `server.go`：HTTP 服务监听 `WEBUI_PORT`（默认 8080，被占用则启动失败），用 Go 标准库 `net/http` + `http.ServeMux`（Go 1.22+ 的方法路由与路径参数），**不引入 gin/echo/chi 等第三方框架**；统一中间件（恢复 panic、请求 ID、超时、JWT 鉴权）用 `http.Handler` 包装函数实现，强类型 DTO + 显式校验函数（不使用反射绑定）；错误映射（`request_id` 透出）。
